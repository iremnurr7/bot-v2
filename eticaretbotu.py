...
def ai_rapor_olustur(df):
    metin = " ".join(df["Mesaj"].astype(str).tail(15))
    prompt = f"İş analisti olarak son 15 mesajı özetle ve patrona 3 aksiyon öner: {metin}"
    try:
...
if mod == "📊 Dashboards":
    ...
    if df is not None:
        # Sekmeli Yapı (Ekran Boşluğunu Doldurur)
        ...
        with tab1:
            ...
            m2.metric("Müşteri Skoru", "4.7/5", "+0.2")
            m3.metric("AI Çözümleme", "%98")
            m4.metric("Sistem Sağlığı", "Optimize")
            
            st.markdown("###")
            
            # Zaman Serisi ve Kategori (Geniş Yerleşim)
            col_trend, col_dist = st.columns([2, 1])
            with col_trend:
                st.markdown("#### Mesaj Yoğunluk Trendi")
                # Basit bir zaman serisi simülasyonu
    ...
    prompt = st.chat_input("Müşteri sorusu simüle edin...")
    if prompt:
